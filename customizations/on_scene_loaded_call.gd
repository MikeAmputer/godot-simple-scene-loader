...
var _on_scene_loaded: Callable = func(_scene): pass
...
func load_scene(
	path: String,
	use_transition_scene: bool = false,
	on_scene_loaded: Callable = func(_scene): pass) -> bool:
	if _is_loading:
...
func _reset() -> void:
	_on_scene_loaded.call(get_tree().current_scene)

	_is_loading = false
	_progress_completion[0] = 0.0
	_on_scene_loaded = func(_scene): pass
	_should_reset = false
